...
function buildMockComparisons(): Comparison[] {
  // Build additional comparisons beyond the 8 in the demo data to enable pagination
  const extra: RecentComparison[] = [
    {
      id: 'cmp-009',
      created_at: (() => { const d = new Date(); d.setDate(d.getDate() - 17); return d.toISOString() })(),
      items_count: 10,
      total_savings: 9.15,
      best_store: 'ALDI',
      items: [
        { name: 'Rice (2 lb)', your_price: 3.99, best_price: 2.49, savings: 1.50 },
        { name: 'Black Beans (15 oz)', your_price: 1.89, best_price: 0.99, savings: 0.90 },
        { name: 'Corn (15 oz)', your_price: 1.69, best_price: 0.89, savings: 0.80 },
      ],
    },
    {
      id: 'cmp-010',
      created_at: (() => { const d = new Date(); d.setDate(d.getDate() - 19); return d.toISOString() })(),
      items_count: 14,
      total_savings: 11.28,
      best_store: 'Walmart',
      items: [
        { name: 'Frozen Pizza (12 in)', your_price: 7.99, best_price: 5.98, savings: 2.01 },
        { name: 'Ice Cream (48 oz)', your_price: 6.49, best_price: 4.98, savings: 1.51 },
        { name: 'Frozen Vegetables (16 oz)', your_price: 2.99, best_price: 1.98, savings: 1.01 },
      ],
    },
    {
      id: 'cmp-011',
      created_at: (() => { const d = new Date(); d.setDate(d.getDate() - 22); return d.toISOString() })(),
      items_count: 5,
      total_savings: 2.76,
      best_store: 'Target',
      items: [
        { name: 'Paper Towels (6 roll)', your_price: 8.99, best_price: 7.49, savings: 1.50 },
        { name: 'Dish Soap (22 oz)', your_price: 3.49, best_price: 2.99, savings: 0.50 },
      ],
    },
    {
      id: 'cmp-012',
      created_at: (() => { const d = new Date(); d.setDate(d.getDate() - 25); return d.toISOString() })(),
      items_count: 18,
      total_savings: 14.33,
      best_store: 'Kroger',
      items: [
        { name: 'Salmon Fillet (1 lb)', your_price: 12.99, best_price: 9.99, savings: 3.00 },
        { name: 'Shrimp (1 lb)', your_price: 10.99, best_price: 8.99, savings: 2.00 },
        { name: 'Cod Fillet (1 lb)', your_price: 8.99, best_price: 6.99, savings: 2.00 },
      ],
    },
    {
      id: 'cmp-013',
      created_at: (() => { const d = new Date(); d.setDate(d.getDate() - 28); return d.toISOString() })(),
      items_count: 9,
      total_savings: 5.87,
      best_store: 'Publix',
      items: [
        { name: 'Rotisserie Chicken', your_price: 8.99, best_price: 6.99, savings: 2.00 },
        { name: 'Cole Slaw (16 oz)', your_price: 3.99, best_price: 2.99, savings: 1.00 },
        { name: 'Dinner Rolls (12 ct)', your_price: 4.49, best_price: 3.49, savings: 1.00 },
      ],
    },
  ]

  const allDemoComparisons = [...DEMO_RECENT_COMPARISONS, ...extra]

  // Build store totals for fake store comparison data
  const storeNames = ['ALDI', 'Walmart', 'Kroger', 'Target', 'Publix']

  return allDemoComparisons.map((dc) => {
    const bestStoreTotal = dc.items.reduce((sum, item) => sum + item.best_price, 0)
    const otherStores = storeNames
      .filter((s) => s !== dc.best_store)
      .slice(0, 2)
      .map((name) => ({
        name,
        total: +(bestStoreTotal + dc.total_savings / 2 + Math.random() * 3).toFixed(2),
        items: dc.items.map((item) => ({
          name: item.name,
          price: +(item.best_price + item.savings * (0.3 + Math.random() * 0.7)).toFixed(2),
        })),
      }))
    ...
    return {
      ...
      items_count: dc.items_count,
      best_option: {
        store: { name: dc.best_store },
        total: +bestStoreTotal.toFixed(2),
        items: dc.items.map((item) => ({
          name: item.name,
      ...
      results: {
        summary: {
          totalItems: dc.items_count,
          itemsFound: dc.items.length,
        },
        stores: [
          {
            name: dc.best_store,
...
// Build monthly chart data from demo data (convert human-readable month names to date strings)
function buildMonthlyData(): MonthlyData[] {
  return DEMO_MONTHLY_SAVINGS.map((m, i) => {
    const d = new Date()
    d.setMonth(d.getMonth() - (DEMO_MONTHLY_SAVINGS.length - 1 - i))
    d.setDate(1)
...
// Build store stats from comparisons
function buildStoreStats(comps: Comparison[]): StoreStats[] {
  ...
  return Array.from(storeStatsMap.entries())
    .map(([name, data]) => ({
      store_name: name,
...
export default function DemoDashboardPage() {
  ...
  const loadData = () => {
    try {
      const mockComparisons = buildMockComparisons()
      const mockMonthly = buildMonthlyData()
      const mockStoreStats = buildStoreStats(mockComparisons)

      // Current month savings from demo stats
      const currentSavings: SavingsData = {
        total_saved: DEMO_DASHBOARD_STATS.total_savings,
        total_spent: DEMO_MONTHLY_SAVINGS[DEMO_MONTHLY_SAVINGS.length - 1].total_spent,
        trips_count: DEMO_DASHBOARD_STATS.comparisons_run,
        avg_savings_per_trip: DEMO_DASHBOARD_STATS.avg_savings_per_trip,
      }

      // Previous month savings for trend comparison
      const prevMonth = DEMO_MONTHLY_SAVINGS[DEMO_MONTHLY_SAVINGS.length - 2]
      const prevSavings: SavingsData = {
        total_saved: prevMonth.total_saved,
        total_spent: prevMonth.total_spent,
        trips_count: prevMonth.trips_count,
        avg_savings_per_trip: +(prevMonth.total_saved / prevMonth.trips_count).toFixed(2),
      }

      // Year-to-date total
      const ytdTotal = DEMO_MONTHLY_SAVINGS.reduce((sum, m) => sum + m.total_saved, 0)

      setSavings(currentSavings)
      setPreviousMonthSavings(prevSavings)
      setHasHistoricalData(true)
      setComparisons(mockComparisons.slice(0, 10))
      setAllComparisons(mockComparisons)
      setReceiptsCount(DEMO_DASHBOARD_STATS.receipts_scanned)
      setCurrentComparisonsCount(DEMO_DASHBOARD_STATS.comparisons_run)
      setPreviousComparisonsCount(Math.round(DEMO_DASHBOARD_STATS.comparisons_run * 0.85))
      setPreviousReceiptsCount(Math.round(DEMO_DASHBOARD_STATS.receipts_scanned * 0.78))
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <button
          onClick={() => router.push('/demo/dashboard/compare')}
        ...
        <button
          onClick={() => router.push('/demo/dashboard/settings')}
          className="p-6 rounded-2xl transition hover:opacity-80 flex items-center justify-between"
          style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)' }}
        >
          <div className="text-left">
                    ...
                    return (
                      <g key={i}>
                        ...
                        <text
                          ...
                          fill="var(--text-muted)"
                        >
                          {new Date(data.month).toLocaleDateString('en-US', { month: 'short', year: '2-digit' })}
                        </text>
                        <text
                          x={x}
                          y={y - 12}
