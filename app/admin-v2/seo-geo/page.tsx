...
import AeoRadarChart from '@/components/admin-v2/charts/AeoRadarChart'
import CroBreakdownChart from '@/components/admin-v2/charts/CroBreakdownChart'
import CroFunnelChart from '@/components/admin-v2/charts/CroFunnelChart'
...
type TabKey = 'overview' | 'pages' | 'technical' | 'content' | 'traffic' | 'geo' | 'aeo' | 'cro' | 'search-console' | 'recommendations'
...
function generateExportReport(audit: StoredAudit | null, recs: StoredRecommendation[]): string {
  ...
  const lines: string[] = [
    ...
    `| AEO Readiness | ${audit.aeo_score ?? 0}/100 |`,
    `| CRO Readiness | ${audit.cro_score ?? 0}/100 |`,
    ``,
    `## Summary`,
  ...
  lines.push(`---`)
  lines.push(`*Generated by Julyu Search & AI Optimization*`)
  return lines.join('\n')
}
      ...
      <div className="flex justify-between items-start mb-8">
        <div>
          ...
          <p style={{ color: 'var(--text-muted)' }}>
            SEO, GEO, AEO & CRO analytics for search engines and AI platforms
          </p>
        </div>
        <button
          onClick={runAudit}
        ...
        <>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                  {[
                    ...
                    { label: 'AEO Readiness', score: audit.aeo_score ?? 0, color: 'text-orange-500' },
                    { label: 'CRO Readiness', score: audit.cro_score ?? 0, color: 'text-teal-500' },
                  ].map(cat => (
          ...
          {activeTab === 'aeo' && (
            <div className="space-y-6">
              {/* AEO Overview */}
              <div className="rounded-2xl p-6" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
                <h3 className="text-lg font-bold mb-2" style={{ color: 'var(--text-primary)' }}>Answer Engine Optimization</h3>
                <p className="text-sm mb-2" style={{ color: 'var(--text-muted)' }}>
                  How well your content is optimized for answer engines like ChatGPT, Perplexity, Google AI Overviews, and voice assistants.
                </p>
                <p className="text-xs italic" style={{ color: 'var(--text-muted)' }}>
                  Unlike GEO (broad AI/LLM optimization), AEO focuses specifically on getting your content selected as direct answers by AI systems.
                </p>
              </div>

              {/* AEO Sub-metric Cards */}
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                {(() => {
                  const avg = (key: keyof StoredPageScore) => {
                    const vals = pageScores.map(p => (p[key] as number) || 0)
                    return vals.length ? Math.round(vals.reduce((s, v) => s + v, 0) / vals.length) : 0
                  }
                  return [
                    { label: 'Schema Richness', score: avg('schema_richness_score') },
                    { label: 'FAQ Coverage', score: avg('faq_coverage_score') },
                    { label: 'Direct Answer Readiness', score: avg('direct_answer_readiness_score') },
                    { label: 'Entity Markup', score: avg('entity_markup_score') },
                    { label: 'Speakable Content', score: avg('speakable_content_score') },
                    { label: 'AI Snippet Compatibility', score: avg('ai_snippet_compatibility_score') },
                  ].map(m => (
                    <div key={m.label} className="rounded-2xl p-5" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
                      <div className="text-xs mb-1" style={{ color: 'var(--text-muted)' }}>{m.label}</div>
                      <div className={`text-2xl font-black ${getScoreColor(m.score)}`}>{m.score}</div>
                      <div className="w-full h-2 rounded-full mt-2" style={{ backgroundColor: 'var(--border-color)' }}>
                        <div
                          className="h-full rounded-full transition-all"
                          style={{
                            width: `${m.score}%`,
                            backgroundColor: m.score >= 71 ? '#10b981' : m.score >= 41 ? '#f59e0b' : '#ef4444',
                          }}
                        />
                      </div>
                    </div>
              ...
              {/* AEO Radar + Per-page Scores */}
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="rounded-2xl p-6" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
                  <h3 className="text-lg font-bold mb-4" style={{ color: 'var(--text-primary)' }}>AEO Dimensions</h3>
                  {(() => {
                    const avg = (key: keyof StoredPageScore) => {
                      const vals = pageScores.map(p => (p[key] as number) || 0)
                      return vals.length ? Math.round(vals.reduce((s, v) => s + v, 0) / vals.length) : 0
                    ...
                    return (
                      <AeoRadarChart
                        schemaRichness={avg('schema_richness_score')}
                        faqCoverage={avg('faq_coverage_score')}
                        directAnswer={avg('direct_answer_readiness_score')}
                        entityMarkup={avg('entity_markup_score')}
                        speakable={avg('speakable_content_score')}
                        aiSnippet={avg('ai_snippet_compatibility_score')}
                      />
                    )
                  })()}
                </div>
                ...
                <div className="rounded-2xl p-6" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
                  <h3 className="text-lg font-bold mb-4" style={{ color: 'var(--text-primary)' }}>Page AEO Scores</h3>
                  <div className="space-y-3 max-h-[400px] overflow-y-auto">
                    {pageScores.map(page => (
                      <div key={page.page_path} className="p-3 rounded-xl" style={{ backgroundColor: 'var(--bg-secondary)' }}>
                        <div className="flex items-center justify-between mb-2">
                          <span className="font-mono text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>{page.page_path}</span>
                          <span className={`text-sm font-bold ${getScoreColor(page.aeo_score || 0)}`}>{page.aeo_score || 0}</span>
                        </div>
                        <div className="grid grid-cols-3 gap-2 text-xs">
                          <div>
                            <span style={{ color: 'var(--text-muted)' }}>Schema: </span>
                            <span className={getScoreColor(page.schema_richness_score || 0)}>{page.schema_richness_score || 0}</span>
                          </div>
                          <div>
                            <span style={{ color: 'var(--text-muted)' }}>FAQ: </span>
                            <span className={getScoreColor(page.faq_coverage_score || 0)}>{page.faq_coverage_score || 0}</span>
                          </div>
                          <div>
                            <span style={{ color: 'var(--text-muted)' }}>Direct: </span>
                            <span className={getScoreColor(page.direct_answer_readiness_score || 0)}>{page.direct_answer_readiness_score || 0}</span>
                          </div>
                          <div>
                            <span style={{ color: 'var(--text-muted)' }}>Entity: </span>
                            <span className={getScoreColor(page.entity_markup_score || 0)}>{page.entity_markup_score || 0}</span>
                          </div>
                          <div>
                            <span style={{ color: 'var(--text-muted)' }}>Speak: </span>
                            <span className={getScoreColor(page.speakable_content_score || 0)}>{page.speakable_content_score || 0}</span>
                          </div>
                          <div>
                            <span style={{ color: 'var(--text-muted)' }}>Snippet: </span>
                            <span className={getScoreColor(page.ai_snippet_compatibility_score || 0)}>{page.ai_snippet_compatibility_score || 0}</span>
                          </div>
                        </div>
                      </div>
                    ))}
              ...
              <div className="rounded-2xl p-6" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
                <h3 className="text-lg font-bold mb-4" style={{ color: 'var(--text-primary)' }}>AEO vs GEO</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-4 rounded-xl" style={{ backgroundColor: 'var(--bg-secondary)' }}>
                    <div className="text-sm font-bold mb-2 text-pink-500">GEO (Generative Engine Optimization)</div>
                    <ul className="text-xs space-y-1" style={{ color: 'var(--text-muted)' }}>
                      <li>Broad AI/LLM citation readiness</li>
                      <li>Content clarity for AI paraphrasing</li>
                      <li>Citation worthiness and authority</li>
                      <li>Schema coverage across page types</li>
                    </ul>
                  </div>
                  <div className="p-4 rounded-xl" style={{ backgroundColor: 'var(--bg-secondary)' }}>
                    <div className="text-sm font-bold mb-2 text-orange-500">AEO (Answer Engine Optimization)</div>
                    <ul className="text-xs space-y-1" style={{ color: 'var(--text-muted)' }}>
                      <li>Direct answer extraction by AI</li>
                      <li>Schema depth and entity richness</li>
                      <li>FAQ structure and Q&A patterns</li>
                      <li>Voice/speakable content readiness</li>
                    </ul>
                  </div>
                </div>
              </div>
              ...
              {/* AEO Recommendations */}
              <div className="rounded-2xl p-6" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
                <h3 className="text-lg font-bold mb-4" style={{ color: 'var(--text-primary)' }}>AEO Improvement Suggestions</h3>
                <div className="space-y-3">
                  {recommendations
                  ...
                  {recommendations.filter(r => r.category === 'aeo').length === 0 && (
                    <p className="text-center py-4 text-green-500 font-semibold">All AEO checks passed!</p>
                  )}
                </div>
              </div>
            </div>
          ...
          {activeTab === 'cro' && (
            <div className="space-y-6">
              {/* CRO Overview */}
              <div className="rounded-2xl p-6" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
                <h3 className="text-lg font-bold mb-2" style={{ color: 'var(--text-primary)' }}>Conversion Rate Optimization</h3>
                <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
                  How effectively your pages drive visitors toward conversion actions — signing up, requesting demos, and engaging with your product.
                </p>
              </div>

              {/* CRO Sub-metric Cards */}
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {(() => {
                  const avg = (key: keyof StoredPageScore) => {
                    const vals = pageScores.map(p => (p[key] as number) || 0)
                  ...
                  return [
                    { label: 'CTA Presence', score: avg('cta_presence_score') },
                    { label: 'Form Accessibility', score: avg('form_accessibility_score') },
                    { label: 'Load Speed Impact', score: avg('load_speed_impact_score') },
                    { label: 'Trust Signals', score: avg('trust_signals_score') },
                    { label: 'Social Proof', score: avg('social_proof_score') },
                    { label: 'Value Proposition', score: avg('value_proposition_score') },
                    { label: 'Mobile CRO', score: avg('mobile_cro_score') },
                  ].map(m => (
                    <div key={m.label} className="rounded-2xl p-5" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
                      <div className="text-xs mb-1" style={{ color: 'var(--text-muted)' }}>{m.label}</div>
                      <div className={`text-2xl font-black ${getScoreColor(m.score)}`}>{m.score}</div>
              ...
              {/* Conversion Funnel */}
              <div className="rounded-2xl p-6" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
                <h3 className="text-lg font-bold mb-4" style={{ color: 'var(--text-primary)' }}>Conversion Readiness Funnel</h3>
                <CroFunnelChart pageScores={pageScores} />
              </div>

              {/* CRO Breakdown + Per-page Analysis */}
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="rounded-2xl p-6" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
                  <h3 className="text-lg font-bold mb-4" style={{ color: 'var(--text-primary)' }}>CRO Breakdown</h3>
                  {(() => {
                    const avg = (key: keyof StoredPageScore) => {
                      const vals = pageScores.map(p => (p[key] as number) || 0)
                      return vals.length ? Math.round(vals.reduce((s, v) => s + v, 0) / vals.length) : 0
                    ...
                    return (
                      <CroBreakdownChart
                        ctaPresence={avg('cta_presence_score')}
                        formAccessibility={avg('form_accessibility_score')}
                        loadSpeed={avg('load_speed_impact_score')}
                        trustSignals={avg('trust_signals_score')}
                        socialProof={avg('social_proof_score')}
                        valueProp={avg('value_proposition_score')}
                        mobileCro={avg('mobile_cro_score')}
                      />
                    )
                  })()}
                </div>
                ...
                <div className="rounded-2xl p-6" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
                  <h3 className="text-lg font-bold mb-4" style={{ color: 'var(--text-primary)' }}>Page CRO Scores</h3>
                  <div className="space-y-3 max-h-[400px] overflow-y-auto">
                    {pageScores.map(page => (
                      <div key={page.page_path} className="p-3 rounded-xl" style={{ backgroundColor: 'var(--bg-secondary)' }}>
                        <div className="flex items-center justify-between mb-2">
                          <span className="font-mono text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>{page.page_path}</span>
                          <span className={`text-sm font-bold ${getScoreColor(page.cro_score || 0)}`}>{page.cro_score || 0}</span>
                        </div>
                        <div className="flex items-center gap-3 text-xs mb-1">
                          <span style={{ color: 'var(--text-muted)' }}>CTAs: <span className="font-semibold" style={{ color: 'var(--text-primary)' }}>{page.cta_count || 0}</span></span>
                          <span style={{ color: 'var(--text-muted)' }}>Forms: <span className="font-semibold" style={{ color: 'var(--text-primary)' }}>{page.form_count || 0}</span></span>
                          <span style={{ color: 'var(--text-muted)' }}>
                            Trust: {page.has_trust_badges ? <span className="text-green-500">Yes</span> : <span className="text-red-500">No</span>}
                          </span>
                          <span style={{ color: 'var(--text-muted)' }}>
                            Social: {page.has_social_proof ? <span className="text-green-500">Yes</span> : <span className="text-red-500">No</span>}
                          </span>
                        </div>
                        {(page.cta_texts || []).length > 0 && (
                          <div className="text-xs truncate" style={{ color: 'var(--text-muted)' }}>
                            CTAs: {(page.cta_texts || []).join(' | ')}
                          </div>
                        )}
                      </div>
                    ))}
              ...
              <div className="rounded-2xl overflow-hidden" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
                <div className="p-4" style={{ borderBottom: '1px solid var(--border-color)' }}>
                  <h3 className="text-lg font-bold" style={{ color: 'var(--text-primary)' }}>Key Conversion Pages</h3>
                </div>
                <table className="min-w-full">
                  <thead>
                    <tr style={{ backgroundColor: 'var(--bg-secondary)' }}>
                      {['Page', 'CRO', 'CTAs', 'Forms', 'Trust', 'Social', 'Value Prop', 'Mobile'].map(h => (
                        <th key={h} className="px-4 py-3 text-left text-xs font-semibold uppercase" style={{ color: 'var(--text-muted)' }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {pageScores
                      .filter(p => ['/', '/pricing', '/for-stores', '/contact', '/features'].includes(p.page_path))
                      .map(page => (
                        <tr key={page.page_path} style={{ borderBottom: '1px solid var(--border-color)' }}>
                          <td className="px-4 py-3 text-sm font-mono" style={{ color: 'var(--text-primary)' }}>{page.page_path}</td>
                          <td className="px-4 py-3">
                            <span className={`px-2 py-1 text-xs font-bold rounded ${getScoreBg(page.cro_score || 0)} ${getScoreColor(page.cro_score || 0)}`}>
                              {page.cro_score || 0}
                            </span>
                          </td>
                          <td className="px-4 py-3 text-sm" style={{ color: 'var(--text-primary)' }}>{page.cta_count || 0}</td>
                          <td className="px-4 py-3 text-sm" style={{ color: 'var(--text-primary)' }}>{page.form_count || 0}</td>
                          <td className="px-4 py-3">{page.has_trust_badges ? <Check /> : <Cross />}</td>
                          <td className="px-4 py-3">{page.has_social_proof ? <Check /> : <Cross />}</td>
                          <td className="px-4 py-3">{page.has_value_prop ? <Check /> : <Cross />}</td>
                          <td className="px-4 py-3">
                            <span className={`text-xs font-bold ${getScoreColor(page.mobile_cro_score || 0)}`}>
                              {page.mobile_cro_score || 0}
                            </span>
                          </td>
                        </tr>
                      ))}
              ...
              {/* CRO Recommendations */}
              <div className="rounded-2xl p-6" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
                <h3 className="text-lg font-bold mb-4" style={{ color: 'var(--text-primary)' }}>CRO Improvement Suggestions</h3>
                <div className="space-y-3">
                  {recommendations
                    .filter(r => r.category === 'cro')
                    .map((rec, i) => (
                      <div key={i} className="flex items-start gap-3 p-3 rounded-xl" style={{ backgroundColor: 'var(--bg-secondary)' }}>
                        <span className={`px-2 py-1 text-xs font-bold rounded shrink-0 ${
                          rec.severity === 'critical' ? 'bg-red-500/15 text-red-500' :
                  ...
                  {recommendations.filter(r => r.category === 'cro').length === 0 && (
                    <p className="text-center py-4 text-green-500 font-semibold">All CRO checks passed!</p>
                  )}
                </div>
              </div>
            </div>
